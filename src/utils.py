...
DYNAMIC_COEFF = ['lambda_mlm', 'lambda_ipm', 'lambda_cmlm']
...
def to_cuda(*args):
    ...
    for x in args:
        if x is None:
            ...
            tmp = [None if i is None else i.cuda() for i in x ]
            x_cuda.append(tmp)
        else:
            x_cuda.append(x.cuda())
    return x_cuda
...
def set_sampling_probs(data, params):
    ...
    if len(params.mono_list) > 0:
        probs = np.array([1.0 * len(data['mono_stream'][lang]['train']) for lang in params.mono_list])  # probs = [2000, 1000]
        probs /= probs.sum()                           # probs = [2/3, 1/3]
        probs = np.array([p ** coeff for p in probs])  # probs = [2/6, 1/6] if coeff = 1/2
        probs /= probs.sum()                           # probs = [2/3, 1/3]
...
def concat_batches(x1, len1, lang1_id, x2, len2, lang2_id, pad_idx, eos_idx, reset_positions, spatial_x1, spatial_x2):
    """
...
def concat_batches_mm(x_w, len_w, m1_id, x_i, len_i, m2_id, 
                      spatial_i=None, params=None):
    """
        Concat batches from different modals.
    """
    
    assert len_w.shape[0] == len_i.shape[0], f"len1: {len1.shape[0]}; len2: {len2.shape[0]}"
    len_w_max = len_w.max().item()
    bs = len_w.size(0)

    positions_w_map = (torch.arange(len_w_max+1) < (len_w[:, None]+1)).long()
    positions_w = (positions_w_map * torch.arange(len_w_max+1).repeat(bs,1)).to(x_w.device)
    positions_w = positions_w.t()
    
#     print(f'len_w: {len_w}')
#     print(f'positions_w: {positions_w.shape}')
#     print(f'positions_w: {positions_w[:,0]}')
#     exit()
    
    assert positions_w.size() == (len_w_max+1, bs)
    x_i = x_i.transpose(0,1)
    x = [x_w, x_i]
    lengths = [len_w, len_i]

    spatial_i = spatial_i.view(-1, bs, len_i[0].item()).permute(2,1,0)
    
    spatial_x = [None, spatial_i]

    return x, spatial_x, lengths, positions_w
